...
<div class="graph-editor-root">
  ...
  <div class="graph-scale-panel" hidden aria-label="Настройка размеров элементов графа">
    ...
    <label>Интервал вершин <output class="graph-node-spacing-value">250</output>
      <input class="graph-node-spacing" type="range" min="60" max="500" step="10" value="250" />
    </label>
  </div>
...
export default function(component) {
  ...
  const labelModes = ["id", "code", "text"];
  let labelMode = labelModes.includes(svg.dataset.labelMode) ? svg.dataset.labelMode : "id";
  ...
  function applyLabelMode() {
    const modeNames = { id: "индексы", code: "коды", text: "описание" };
